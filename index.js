...
function preload() {
  ...
  desenfoque = loadFont('/assets/DESENFOQUE.ttf');
}
...
fontCheckbox = createCheckbox('', false);
fontCheckbox.position(windowWidth / 2 - 80, windowHeight / 2 + 187 );
...
slider.position(windowWidth/2+10, windowHeight/2+230);
  ...
  myImput.position(windowWidth/2-50, windowHeight/2+190)
 
  col1 = "black"
  col2 = "grey"

  downloadButton = createButton('Descargar fuentes');
downloadButton.position(windowWidth- 200, 50);
downloadButton.mousePressed(downloadFonts);
...
function downloadFonts() {
  ...
  // ENFOQUE
  let a = createA('/assets/ENFOQUE.ttf', '');
  a.attribute('download', 'ENFOQUE.ttf');
  a.hide();
  a.elt.click();

  // DESENFOQUE
  setTimeout(() => {
    let b = createA('/assets/DESENFOQUE.ttf', '');
    b.attribute('download', 'DESENFOQUE.ttf');
    b.hide();
    b.elt.click();
  }, 300);
}